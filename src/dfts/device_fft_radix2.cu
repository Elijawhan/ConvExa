...
__global__ void device_fft_radix2(const T* signal, const size_t length, cuda::std::complex<T>* result)
{
    ...
    // In-place FFT
    for (uint32_t idx = 1; idx <= __log2(static_cast<float>(length)); idx++)
    {
        ...
        for (jdx = index; jdx < length; jdx += blockDim.x*gridDim.x)
        {
            int group = jdx / block;
            int start = group * block;
            int rel_pos = jdx & block;

            // Typically, kdx == index, but this may not be true later on, so we calculate it
            int32_t kdx = start + rel_pos;
            ...
            if (rel_pos < half)
            {
                ...
                cuda::std::complex<T> twiddle = twiddle_factor(rel_pos, block);
